...
export class NeuronalSpider {
	...
	public points: Point[];
	public targetMouse: IGeometricPoint;
	...
	public interchangeBackground() {
		let style: CSSStyleDeclaration = window.getComputedStyle(this.configuration.targetElement);
		if (style.background != "") {
	...
	public createCanvas(): void {
		let clientRect: ClientRect = this.configuration.targetElement.getBoundingClientRect();
	...
	private animate(): void {
		if (this.animateHeader) {
			...
			for (let i in this.points) {
				let maxOpacity: number = this.configuration.maximumOpacity;
				let distance: number = Point.getAbsolutDistance(this.targetMouse, this.points[i]);
